...
// Theme Custom Code
$cus_class = $prgs_rand_id = ''; $cus_stat = 0;
if( isset( $bar_height ) && $bar_height != '' ){
	$prgs_rand_id = 'megafactory-custom-progress-' . megafactory_shortcode_rand_id();
	$cus_class = ' megafactory-inline-css ' . $prgs_rand_id;
	$cus_stat = 1;
}
$cus_class .= isset( $bar_style ) ? ' vc-progress-bar-' . esc_attr( $bar_style ) : '';

$class_to_filter = 'vc_progress_bar wpb_content_element' . $cus_class;
...
// Theme Custom Code
if( $cus_stat ){
	$shortcode_css = '.' . esc_attr( $prgs_rand_id ) . ' > .vc_single_bar { height: '. absint( $bar_height ) .'px; line-height: '. absint( $bar_height ) .'px; }';
	$wrapper_attributes[] = 'data-css="' . htmlspecialchars( json_encode( $shortcode_css ), ENT_QUOTES, 'UTF-8' ) . '"';
}
