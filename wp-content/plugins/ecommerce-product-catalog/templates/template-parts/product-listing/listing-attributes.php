...
if ( $attributes_number > 0 && has_product_any_attributes( $product_id ) ) {
	$max_listing_attributes	 = apply_filters( 'max_product_listing_attributes', $listing_attributes_num );
	?>
	<div class="product-attributes">
		...
		for ( $i = 1; $i <= $attributes_number; $i++ ) {
			...
			if ( !empty( $attribute_value ) ) {
				?>
				<div><span class="attribute-label-listing"><?php echo get_attribute_label( $i, $product_id ) ?>:</span> <span class="attribute-value-listing"><?php echo get_attribute_value( $i, $product_id ) ?></span> <span class="attribute-unit-listing"><?php echo get_attribute_unit( $i, $product_id ) ?></span></div>
					<?php
					$a++;
				}
