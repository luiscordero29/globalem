jQuery(function($) {
	...
	/*Metabox Tab*/
	$(".megafactory-admin-tabs-menu a").click(function(event) {
        event.preventDefault();
        $(this).parent().addClass("current");
        $(this).parent().siblings().removeClass("current");
        var tab = $(this).attr("href");
        $(this).parents(".megafactory-admin-tabs-menu").next(".megafactory-admin-tab").find(".megafactory-admin-tab-content").not(tab).css("display", "none");
        $(tab).fadeIn();
    });
	
	/*Megafactory Switcher*/
	$('.megafactory-switcher').change(function() {
        if($(this).is(":checked")) {
			$(this).parents('.megafactory-switch').find('.megafactory-switcher-stat').val('1');
        }else{
			$(this).parents('.megafactory-switch').find('.megafactory-switcher-stat').val('0');
		}
    });
	
	/*Megafactory Color Picker*/
	if( $(".megafactory-color-picker").length ){
		
		var patt = /^#([\da-fA-F]{2})([\da-fA-F]{2})([\da-fA-F]{2})$/;
		var color, rgb, matches; 
		$(".megafactory-color-picker").wpColorPicker({
			clear: function() { $(this).parents('td').find('.megafactory-alpha-color').val(''); },
			change: function (event, ui) {
				...
				$(this).parents('td').find('.megafactory-alpha-color').val(rgb);
				//alert(color);
				// Add your code here
			}
		});
		...
		$( ".alpha-range" ).change(function() {
			var opacity = $(this).val(); //megafactory-alpha-color-picker
			color = $(this).parents('td').find('.megafactory-alpha-color-picker').val();
			if( rgb != '' ){
				matches = patt.exec(color);
				rgb = "rgba("+parseInt(matches[1], 16)+","+parseInt(matches[2], 16)+","+parseInt(matches[3], 16)+","+ parseFloat(opacity/10) +")";
				$(this).parents('td').find('.megafactory-alpha-color').val(rgb);
