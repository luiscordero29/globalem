...
define( 'MEGAFACTORY_METABOXES_DIR', MEGAFACTORY_CORE_URL . 'admin/metabox/metaboxes' );
...
function megafactory_meta_box_unique_key() {
	static $meta_key = 1;
	return $meta_key++;
} 

function megafactory_post_option_get_sidebars( $name, $selected ){
	$output = '<select name="'. esc_attr( $name ) .'">';
		$output .= '<option value="">'. esc_html__( 'None', 'megafactory-core' ) .'</option>';
...
function megafactory_post_option_drag_drop($post_items, $post_items_array, $status) {
...
function megafactory_post_option_drag_drop_multi( $key, $post_items ) {
...
function megafactory_post_option_image_select( $field, $values, $default, $title) {
...
function megafactory_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'switch':
			...
			$output .= '
				<div class="megafactory-switch">
					<label class="switch">
						<input type="checkbox" class="megafactory-switcher" '. ( $def == '1' ? 'checked' : '' ) .'>
						<div class="slider round"></div>
					</label>
					<input type="hidden" class="megafactory-switcher-stat" name="' . esc_attr( $name ) . '" value="'. esc_attr( $def ) .'" />
				</div><br />' . $desc;
		break;
		case 'dimension':
			$space_values = array();
			...
			$output .= '<div class="spacing-parent">
  					...
  					$output .= '<input size="5" type="text" name="' . esc_attr( $name .'_0') . '" value="'. esc_attr( $space_values[0] ) .'">
							...
							<select name="' . esc_attr( $name .'_1' ) . '">
								<option value="px" '. ( $space_values['1'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'megafactory-core' ) .'</option>
								<option value="em" '. ( $space_values['1'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'megafactory-core' ) .'</option>
								<option value="percent" '. ( $space_values['1'] == 'percent' ? 'selected' : '' ) .'>%</option>
							</select>
						</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="megafactory_dimension" /><br />' . $desc;
		break;
		case 'space':
			$space_values = array();
			...
			$output .= '<div class="spacing-parent">
							...
							<select name="' . esc_attr( $name .'_4' ) . '">
								<option value="px" '. ( $space_values['4'] == 'px' ? 'selected' : '' ) .'>'. esc_html__( 'px', 'megafactory-core' ) .'</option>
								<option value="em" '. ( $space_values['4'] == 'em' ? 'selected' : '' ) .'>'. esc_html__( 'em', 'megafactory-core' ) .'</option>
								<option value="percent" '. ( $space_values['4'] == 'percent' ? 'selected' : '' ) .'>%</option>
							</select>';
							if( $border_style ){
								$output .= '<select name="' . esc_attr( $name .'_6' ) . '">
									<option value="solid" '. ( $space_values['6'] == 'solid' ? 'selected' : '' ) .'>'. esc_html__( 'Solid', 'megafactory-core' ) .'</option>
									<option value="dashed" '. ( $space_values['6'] == 'dashed' ? 'selected' : '' ) .'>'. esc_html__( 'Dashed', 'megafactory-core' ) .'</option>
									<option value="dotted" '. ( $space_values['6'] == 'dotted' ? 'selected' : '' ) .'>'. esc_html__( 'Dotted', 'megafactory-core' ) .'</option>
									<option value="double" '. ( $space_values['6'] == 'double' ? 'selected' : '' ) .'>'. esc_html__( 'Double', 'megafactory-core' ) .'</option>
									<option value="none" '. ( $space_values['6'] == 'none' ? 'selected' : '' ) .'>'. esc_html__( 'None', 'megafactory-core' ) .'</option>
								</select>';
							}else{
								$output .= '<input type="hidden" name="' . esc_attr( $name .'_6' ) . '" value="" />';
							}
							if( $color_opt ){
								$output .= '<input class="megafactory-color-picker" id="' . esc_attr( $id.'_5' ) . '" name="' . esc_attr( $name.'_5' ) . '" value="' . esc_attr( $space_values[5] ) . '" type="text" size="10" />';
							}else{
								$output .= '<input type="hidden" name="' . esc_attr( $name .'_5' ) . '" value="" />';
							}
			$output .= '</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="megafactory_space" /><br />' . $desc;
		...
		case 'link_color':
			...
			$output .= '<div class="link-color-parent">
							<i>'. esc_html__( 'Regular', 'megafactory-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_0' ) . '" class="megafactory-color-picker" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $link_values[0] ) . '" size="10"  />
							<i>'. esc_html__( 'Hover', 'megafactory-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_1' ) . '" class="megafactory-color-picker" id="' . esc_attr( $id.'_1' ) . '" value="' . esc_attr( $link_values[1] ) . '" size="10"  />
							<i>'. esc_html__( 'Active', 'megafactory-core' ) .'</i><input type="text" name="' . esc_attr( $name.'_2' ) . '" class="megafactory-color-picker" id="' . esc_attr( $id.'_2' ) . '" value="' . esc_attr( $link_values[2] ) . '" size="10"  />
						</div>
						<input type="hidden" name="' . esc_attr( $name ) . '" value="megafactory_link_color" /><br />' . $desc;
		...
		case 'alpha_color':
			...
			$output .= '<div class="megafactory-alpha-color">';
				$output .= '<input type="text" name="' . esc_attr( $name.'_0' ) . '" id="' . esc_attr( $id.'_0' ) . '" value="' . esc_attr( $alpha_values[0] ) . '" class="megafactory-alpha-color" />
						<input class="megafactory-color-picker megafactory-alpha-color-picker" id="' . esc_attr( $id.'_0' ) . '" name="' . esc_attr( $name.'_1' ) . '" value="' . esc_attr( $alpha_values[1] ) . '" type="text" size="10" />
						<input type="range" min="1" max="10" step="1" value="3" class="alpha-range">';
				$output .= '<input type="hidden" name="' . esc_attr( $name ) . '" value="megafactory_alpha_color" /><br />' . $desc;
			$output .= '</div>';
		break;
		...
		break;
		case 'rating':
			$output .= '<ul class="megafactory-meta-rating star-rating">
						...
						<input type="hidden" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . absint( $meta ) . '" class="megafactory-meta-rating-value" />
					<br />' . $desc;
		break;
		case 'number':
		...
		// color
		case 'color':
			$output .= '<input type="text" name="' . esc_attr( $name ) . '" class="megafactory-color-picker" id="' . esc_attr( $id ) . '" value="' . esc_attr( $meta ) . '" size="10"  />';
		break;
		...
		// image
		case 'image':
			$image = MEGAFACTORY_METABOXES_DIR . '/images/image.png';	
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					$output .= megafactory_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					$output .= '</p>';
				} // end each field
		...
		case 'dragdrop':
			...
			$t_meta_key = megafactory_meta_box_unique_key();
			$meta = isset( $meta ) && $meta != '' ? $meta : $default;
			...
			$post_enabled_out = $post_enabled_items != '' ? megafactory_post_option_drag_drop( $post_enabled_items, $options['items'], 'enabled' ) : '';
			$post_disabled_out =  $post_disabled_items != '' ? megafactory_post_option_drag_drop( $post_disabled_items, $options['items'], 'disabled' ) : '';
			
			$output .= '<div class="meta-drag-drop-field" data-field="'. $t_meta_key .'">';
				$output .= '<h4>'. esc_html__( 'Enabled Items', 'megafactory-core' ) .'</h4>';
				$output .= $post_enabled_out;
				$output .= '<h4>'. esc_html__( 'Disabled Items', 'megafactory-core' ) .'</h4>';
		...
		case 'dragdrop_multi':
			...
			foreach( $part_array as $key => $value ){
				$t_part_array[$key] = $value != '' ? megafactory_post_option_drag_drop_multi( $key, $value ) : '';
			}
		...
		case 'image_select':
			...
			$output .= megafactory_post_option_image_select( $name, $options, $selected, $label );
		...
		case 'sidebar':
			...
			$selected = isset( $meta ) && $meta != '' ? $meta : $default;
			$output .= megafactory_post_option_get_sidebars( $name, $selected);
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			if ( meta_box_find_field_type( 'color', $this->fields ) 
					|| meta_box_find_field_type( 'link_color', $this->fields )
					|| meta_box_find_field_type( 'alpha_color', $this->fields ) ){
			...
			if ( meta_box_find_field_type( 'alpha_color', $this->fields ) ){
				wp_enqueue_style( 'rangeslider', MEGAFACTORY_METABOXES_DIR . '/css/rangeslider.css', array(), '1.0' );
				wp_enqueue_script( 'jquery-rangeslider', MEGAFACTORY_METABOXES_DIR . '/js/rangeslider.min.js', array( 'jquery' ), '1.0', true );
			}
			

			wp_enqueue_script( 'meta_box', MEGAFACTORY_METABOXES_DIR . '/js/scripts.js', $deps );
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', MEGAFACTORY_METABOXES_DIR . '/css/jqueryui.css' );
			if ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', MEGAFACTORY_METABOXES_DIR . '/css/meta_box.css', $deps );
		}
	}
	...
	function meta_box_callback() {
		...
		wp_nonce_field( 'megafactory_meta_box_nonce_action', 'megafactory_meta_box_nonce_field' );
		...
		foreach ( $this->fields as $field) {
			...
			if( isset( $field['required'] ) )
				...
				if( $field['type'] != 'label' && $field['type'] != 'line' ){
					...
					$meta_output[$tab] .= megafactory_meta_box_field( $field, $meta  );
		...
		foreach($meta_output as $key => $value){
			...
			$tab_list .= '<li class="'. ( $i == 0 ? 'current' : '' ) .'"><a href="#megafactory-admin-tab-'. esc_attr( $tab_id ) .'">'. esc_html( $key ) .'</a></li>';
			$tab_content .= '<div id="megafactory-admin-tab-'. esc_attr( $tab_id ) .'" class="megafactory-admin-tab-content'. ( $i == 0 ? ' current' : '' ) .'">';
		...
		echo '
		<div id="megafactory-admin-tabs-container">
			<ul class="megafactory-admin-tabs-menu">
				'. $tab_list .'
			</ul>
			<div class="megafactory-admin-tab">
				'. $tab_content .'
			</div>
		</div>';
	}
	...
	function save_box( $post_id ) {
		...
		// verify nonce
		if ( ! isset( $_POST['megafactory_meta_box_nonce_field'] ) )
			return $post_id;
		if ( ! ( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['megafactory_meta_box_nonce_field'],  'megafactory_meta_box_nonce_action' ) ) ) 
			return $post_id;
		// check autosave
