...
$cols = $aps->megafactoryGetThemeOpt( 'portfolio-grid-cols' );
$gutter = $aps->megafactoryGetThemeOpt( 'portfolio-grid-gutter' );
$infinite = $aps->megafactoryGetThemeOpt( 'portfolio-infinite-scroll' ) ? 'true' : 'false';
$isotope = $aps->megafactoryGetThemeOpt( 'portfolio-grid-type' );
$extra_class .= $aps->megafactoryGetThemeOpt( 'portfolio-grid-type' ) == 'normal' ? ' grid-normal' : '';
		...
		<main id="main" class="site-main archive-template <?php echo esc_attr( $extra_class ); ?>" role="main" data-cols="<?php echo esc_attr( $cols ); ?>" data-gutter="<?php echo esc_attr( $gutter ); ?>">
			...
			if ( $query->have_posts() ) {
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'post megafactory-portfolio' ); ?>>
