...
class Zozo_Tweets_Widget extends WP_Widget {
	...
	public function __construct() {
		/* Widget settings. */
		$widget_options = array('classname' => 'zozo_tweets_widget', 'description' => esc_html__( 'Displays Twitter feeds.', 'megafactory-core' ) );
		$control_options = array('id_base' => 'zozo_tweets-widget');
		
		/* Create the widget. */
		parent::__construct('zozo_tweets-widget', esc_html__( 'Megafactory Twitter Feeds', 'megafactory-core' ), $widget_options, $control_options);
	}
...
function megafactory_get_tweets( $consumer_key, $consumer_secret, $access_token, $access_token_secret, $tweet_count, $twitter_id ){
	// Include Main Library File
	require_once( MEGAFACTORY_CORE_DIR . 'widgets/twitter/twitteroauth.php' );
	...
	if( ( false === $cached_twitter_feeds || empty( $cached_twitter_feeds ) ) || $tweet_count > count( $cached_twitter_feeds ) ) {
		...
		// Get Access Token
		$connection = megafactoryGetConnectionWithAccessToken($consumer_key, $consumer_secret, $access_token, $access_token_secret);				
		$params = array(
		  'count' 		=> $tweet_count,
...
function megafactory_tweet_time_ago( $time ) {
...
function megafactoryGetConnectionWithAccessToken($cons_key, $cons_secret, $oauth_token, $oauth_token_secret) 
{
