<?php
add_action( 'widgets_init', 'megafactory_zozo_contact_info_widget' );
function megafactory_zozo_contact_info_widget() {
	register_widget( 'megafactory_zozo_contact_infos_widget' );
}
class megafactory_zozo_contact_infos_widget extends WP_Widget {
	/**
	 * Widget setup.
	 */
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zozo_contact_info_widget', 'description' => esc_html__('A widget that displays an About widget', 'megafactory-core') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'zozo_contact_info_widget' );
		/* Create the widget. */
		parent::__construct( 'zozo_contact_info_widget', esc_html__('Megafactory Contact Info', 'megafactory-core'), $widget_ops, $control_ops );
		...
		?>
			...
			<div class="contact-widget widget-content">
				...
				<div class="contact-widget-info">
					<?php if($caddress) : ?>
					<p class="contact-address"><span class="icon-directions icons"></span><span><?php echo wp_kses_post($caddress); ?></span></p>
					<?php endif; ?>	
					
					<?php if($cphone) : ?>
					<p class="contact-phone"><span class="icon-screen-smartphone icons"></span><span><?php echo esc_attr($cphone); ?></span></p>
					<?php endif; ?>	
					
					<?php if($cmail) : ?>
					<p class="contact-email"><span class="icon-envelope icons"></span><span><a href="mailto:<?php echo esc_attr($cmail); ?>"><?php echo esc_attr($cmail); ?></a></span></p>
					<?php endif; ?>	
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__( 'Contact Info', 'megafactory-core' ), 'ctext' => '', 'caddress' => '', 'cphone' => '', 'cmail' => '');
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'ctext' ) ); ?>"><?php esc_html_e( 'Text Message', 'megafactory-core' ); ?></label>
		...
		<!-- Address -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'caddress' ) ); ?>"><?php esc_html_e( 'Address', 'megafactory-core' ); ?></label>
		...
		<!-- Phone Numbers -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cphone' ) ); ?>"><?php esc_html_e( 'Phone Numbers', 'megafactory-core' ); ?></label>
		...
		<!-- Email -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'cmail' ) ); ?>"><?php esc_html_e( 'Email', 'megafactory-core' ); ?></label>
